...
> **Moved here** (issue #32). This verifies the **API's** relay (`api/src/live/`),
> so this is its home. It was first committed under `orchestrator/scripts/` because
> the agent that ran #32 was scoped there while another was working in `api/`, and
> leaving it uncommitted would have made the verification irreproducible. A
> coordinator pass moved it; nothing inside it referenced the orchestrator, so the
> move was a relocation and not a port.
